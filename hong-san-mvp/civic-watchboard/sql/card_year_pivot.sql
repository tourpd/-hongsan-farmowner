WITH base AS (
  SELECT bidNtceDt AS dt, bidNtceNm AS title
  FROM bids
),
tagged AS (
  SELECT
    CASE
      ...
      ELSE '정책카드: 기타'
    END AS policy,
    substr(dt,1,4) AS yyyy
  FROM base
),
year_counts AS (
  SELECT policy, yyyy, COUNT(*) AS cnt
  FROM tagged
  GROUP BY policy, yyyy
)
SELECT
  policy,
  COALESCE(SUM(CASE WHEN yyyy='2022' THEN cnt END),0) AS y2022,
  COALESCE(SUM(CASE WHEN yyyy='2023' THEN cnt END),0) AS y2023,
  COALESCE(SUM(CASE WHEN yyyy='2024' THEN cnt END),0) AS y2024,
  COALESCE(SUM(CASE WHEN yyyy='2025' THEN cnt END),0) AS y2025,
  COALESCE(SUM(CASE WHEN yyyy='2026' THEN cnt END),0) AS y2026,
  COALESCE(SUM(cnt),0) AS total
FROM year_counts
GROUP BY policy
ORDER BY total DESC, policy;
